...
def sky_subtraction(input_filename,output_filename,sky_fibers_file):
	...
	if plot == 'plot_not':
		...
		for index in range(number_of_fibers):
	...
	for index, spectrum in enumerate(input_data):
		...
		if sigma < cdelt/4:
			sigma = cdelt/3
		#print(sigma)
		before_peak = range(int(round((peak-10*sigma))),int(round((peak-5*sigma))))
		after_peak = range(int(round((peak+5*sigma))),int(round((peak+10*sigma))))
		...
		if len(linear_pixels) > 1:
			...
			#plt.show()
			try:
				coeff, gauss_matrix = curve_fit(gauss, range(number_of_pixels), offset_spectrum, p0=p0)
				gauss_fit = gauss(range(number_of_pixels), *coeff)
			except:
	...
	for spectrum_index in range(number_of_pixels):
		for fiber_index in range(sky_fibers[0],sky_fibers[1]):
	...
	for index, spectrum in enumerate(input_data):
		if plot == 'plot':
				...
				plt.plot(range(number_of_pixels),input_data[index,:])
				#input_data[index,:] = input_data[index,:]-average_sky_vector
				plt.plot(range(number_of_pixels),input_data[index,:]-average_sky_vector)
				plt.plot(range(number_of_pixels),average_sky_vector)
				plt.ylim(0,5000)
				plt.draw()
				#time.sleep(sleep_time)
